...
public class Registration_2Test extends InitializationB {

	// Test Goal:Missing First Name field
	...
	@Test
	public void Reg_FirstName2() throws IOException {
		...
		DDT excel = new DDT();
		ArrayList x = excel.getData("FirstName2");

		// Make object of the home page
		TravelsRegisterationPage RegPage = new TravelsRegisterationPage(driver);

		// Enter the last name data
		RegPage.getLastName().sendKeys(x.get(1).toString());

		// Enter the mobile number data
		RegPage.getPhone().sendKeys(x.get(2).toString());

		// Enter the Email address data
		RegPage.getEmail().sendKeys(x.get(3).toString());

		// Enter the Password data
		RegPage.getPassword().sendKeys(x.get(4).toString());

		// Confirm the Password data
		RegPage.getConfirmpassword().sendKeys(x.get(5).toString());
		...
		// Check the error MSG
		error_Message = RegPage.getFirstName().getAttribute("validationMessage");
