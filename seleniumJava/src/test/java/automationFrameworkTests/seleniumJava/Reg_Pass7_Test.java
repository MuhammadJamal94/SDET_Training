...
public class Reg_Pass7_Test extends InitializationB {

	// Test Goal:Check error MSG will appear in case password and confirm password do not match
	// characters
	...
	public void Reg_Pass1() throws IOException {
		...
		DDT excel = new DDT();
		ArrayList x = excel.getData("Password7");
		...
		// This will scroll the page till the element is found
		js.executeScript("arguments[0].scrollIntoView();",RegPage.EmailAlert());
		// Click on start BTN

		// Check the error MSG
		error_Message = RegPage.EmailAlert().getText();
		System.out.println("THIS IS THE MSG " + error_Message);

		// Check MSG
		Assert.assertEquals(error_Message, "Password not matching with confirm password.");
	}
